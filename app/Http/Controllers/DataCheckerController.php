...
class DataCheckerController extends Controller
{
    public function checkDataDates(Request $request)
    {
        ...
        $models = [
            ...
            'tiktok_gmv' => TiktokGmvData::class,
            'tiktok_live_streaming_own' => [
                'model' => TiktokLiveStreamingData::class,
                'type' => 'own',
            ],
            'tiktok_live_streaming_affiliate' => [
                'model' => TiktokLiveStreamingData::class,
                'type' => 'affiliate',
            ],
            'tiktok_video_analytics_own' => [
                'model' => TiktokVideoAnalyticsData::class,
                'type' => 'own',
            ],
            'tiktok_video_analytics_affiliate' => [
                'model' => TiktokVideoAnalyticsData::class,
                'type' => 'affiliate',
            ],
        ...
        foreach ($models as $type => $modelData) {
            // Cek apakah $modelData adalah array (untuk tipe dengan filter type)
            if (is_array($modelData)) {
                $model = $modelData['model'];
                $typeValue = $modelData['type'];
            } else {
                $model = $modelData;
                $typeValue = null;
            }
            ...
            foreach ($brands as $brand) {
                if ($hasMarketPlaceId) {
                    foreach ($marketPlaces as $marketPlace) {
                        ...
                        $existingDatesQuery = $model::where('brand_id', $brand->id)
                            ->where('market_place_id', $marketPlace->id)
                            ->whereBetween('data_date', [$startDate, $endDate]);

                        // Tambahkan filter type jika ada
                        if ($typeValue) {
                            $existingDatesQuery->where('type', $typeValue);
                        }

                        $existingDates = $existingDatesQuery->distinct()
                            ->pluck('data_date')
                            ->map(function ($date) {
